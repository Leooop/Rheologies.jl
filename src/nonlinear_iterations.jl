function nonlinear_solve!(u, δu, model::Model{dim,N,D,V,E,P,NewtonRaphson}, restart_flag) where {dim,N,D,V,E,P}
    ...
    while true; newton_itr += 1
        ...
        @timeit "assemble" doassemble!(model::Model{dim,N,Nothing,Nothing,E,P}, nbasefuncs, u ; elastic_only = (newton_itr == 0))

        norm_r = norm(res[JuAFEM.free_dofs(dbc)])

        print("Iteration: $newton_itr \tresidual: $(@sprintf("%.5f", norm_r))\n")

        #### EXIT CHECKS ####
        if norm_r < solver.atol
            ## output
            vals = output_values(model)
            if clock.iter >= 1
                output(vals, u, dh, dbc, clock)
            end
            break
        end
