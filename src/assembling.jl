...
function doassemble(cellvalues_u::CellVectorValues{dim}, cellvalues_p::CellScalarValues{dim},
                    ...
                    dh::DofHandler, bc_dicts::BoundaryConditions, rheology::Rheology) where {dim}
    ...
    for cell in CellIterator(dh)
        fill!(ke, 0)
        fill!(fe, 0)
        assemble_up!(ke, fe, cell, cellvalues_u, cellvalues_p, facevalues_u, grid, rheology, bc_dicts, ɛdev)
...
function assemble_up!(Ke, fe, cell, cellvalues_u, cellvalues_p, facevalues_u, grid, rheology, bc_dicts, ɛdev)
...
"""
function apply_Neumann_bc!(fe, cell::CellIterator, grid::Grid, bc_dicts::BoundaryConditions, facevalues, n_basefuncs)
    t = 0.0 # TODO : make use of simulation time when it will be implemented
    bc_dicts.neumann != nothing && @inbounds for face in 1:nfaces(cell)
        if onboundary(cell, face)
            for (neumann_set, traction_func) in pairs(bc_dicts.neumann)
